...
while [ $n -lt $max_wait ]; do
    NOT_READY=$(kubectl get pods -n mender -o "custom-columns=NAMESPACE:metadata.namespace,POD:metadata.name,READY-true:status.containerStatuses[*].ready" | grep -v 'mender-deployments-storage-daemon' | grep -v '-test-' | egrep -e 'false$' -e '<none>$' | wc -l)
